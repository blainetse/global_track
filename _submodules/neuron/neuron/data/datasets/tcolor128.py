...
class TColor128(SeqDataset):
    ...
    def __init__(self, root_dir=None, download=True):
        ...
        super(TColor128, self).__init__(name='TColor-128', root_dir=root_dir)

    def _construct_seq_dict(self, root_dir):
        # image and annotation paths
        anno_files = sorted(glob.glob(osp.join(root_dir, '*/*_gt.txt')))
        seq_dirs = [osp.dirname(f) for f in anno_files]
        seq_names = [osp.basename(d) for d in seq_dirs]
        # valid frame range for each sequence
        range_files = [
            glob.glob(osp.join(d, '*_frames.txt'))[0] for d in seq_dirs
        ]
        ...
        for s, seq_name in enumerate(seq_names):
            ...
            frames = np.loadtxt(range_files[s], dtype=int, delimiter=',')
            img_files = [
                osp.join(seq_dirs[s], 'img/%04d.jpg' % f)
