...
class MOT(SeqDataset):
    ...
    def __init__(self, root_dir=None, version=2016, subset='train'):
        ...
        # initialize dataset
        super(MOT, self).__init__(self.name,
    ...
    def _construct_seq_dict(self, root_dir, version, subset):
        ...
        seq_dirs = sorted(glob.glob(osp.join(root_dir, subset, '*/img1')))
        parent_dirs = [osp.dirname(d) for d in seq_dirs]
        seq_names = [osp.basename(d) for d in parent_dirs]
        det_files = [osp.join(d, 'det/det.txt') for d in parent_dirs]
        if subset == 'train':
            gt_files = [osp.join(d, 'gt/gt.txt') for d in parent_dirs]
        ...
        for s, seq_name in enumerate(seq_names):
            ...
            det = np.loadtxt(det_files[s], delimiter=',', dtype=np.float32)
            det = self._format_det(det)
            target = {'det': det}

            if subset == 'train':
                gt = np.loadtxt(gt_files[s], delimiter=',', dtype=np.float32)
            ...
            # update seq_dict
            seq_dict[seq_name] = {'img_files': img_files, 'target': target}
    ...
    def _format_gt(self, gt):
        ...
        mask_class = np.logical_or.reduce(
            (anno[:, 7] == 0, anno[:, 7] == 2, anno[:, 7] == 3, anno[:,
                                                                     7] == 4))
