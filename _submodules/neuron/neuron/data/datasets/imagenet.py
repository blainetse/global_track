...
class ImageNetVID(SeqDataset):
    ...
    CLASSES = ('airplane', 'antelope', 'bear ', 'bicycle', 'bird', 'bus',
               ...
               'motorcycle ', 'rabbit', 'red panda', 'sheep', 'snake',
               'squirrel', 'tiger', 'train', 'turtle', 'watercraft ', 'whale',
               'zebra')
    WORDNET_IDs = ('n02691156', 'n02419796', 'n02131653', 'n02834778',
    ...
    def __init__(self, root_dir=None, subset=['train', 'val']):
        ...
        # initialize dataset
        super(ImageNetVID, self).__init__(self.name,
            ...
            for f, group in enumerate(objects):
                for obj in group:
                    anno_obj = [
                        ...
                        1,  # score/keep
                        self.WORDNET_IDs.index(
                            obj.find('name').text),  # class_id
                        int(obj.find('occluded').text)
                    ]  # occlusion
