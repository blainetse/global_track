...
class QG_RPN(BaseDetector, RPNTestMixin):
    ...
    def init_weights(self, pretrained=None):
        ...
        # with_neck implemented in BaseDetector
        if self.with_neck:
    ...
    def forward_dummy(self, *args, **kwargs):
        raise NotImplementedError("forward_dummy is not implemented for QG_RPN")
    ...
    def forward_train(
        ...
        z = self.extract_feat(img_z)  # the ROI (Region-Of-Interest) features of the query instance
        x = self.extract_feat(img_x)  # the search image features

        losses = {}
        num = 0.0  # total number of instances
        # TODO: 这一步如何实现 x z 的融合的？在 forward 方法里面
    ...
    def forward_test(self, img_z, img_x, img_meta_z, img_meta_x, gt_bboxes_z, **kwargs):
        # assume one image and one query instance only
        return self.simple_test(
            img_z, img_x, img_meta_z, img_meta_x, gt_bboxes_z, **kwargs
        )
    ...
    def aug_test(self, imgs, img_metas, rescale=False):
        raise NotImplementedError("aug_test is not implemented for QG-RPN")
