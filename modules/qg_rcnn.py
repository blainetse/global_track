...
class QG_RCNN(TwoStageDetector):
    ...
    def forward_dummy(self, *args, **kwargs):
        raise NotImplementedError("forward_dummy is not implemented for QG_RCNN")
    ...
    def forward_train(
        ...
        for x_ij, i, j in self.rpn_modulator(z, x, gt_bboxes_z):
            ...
            rpn_loss_inputs = rpn_outs + (gt_bboxes_ij, img_meta_xi, self.train_cfg.rpn)
            ...
            assign_result = bbox_assigner.assign(
                proposal_list[0], gt_bboxes_ij[0], gt_bboxes_ignore[0], gt_labels_ij[0]
            )
            ...
            bbox_targets = self.bbox_head.get_target(
                sampling_results, gt_bboxes_ij, gt_labels_ij, self.train_cfg.rcnn
            )
    ...
    def simple_test(self, img_z, img_x, img_meta_z, img_meta_x, gt_bboxes_z, **kwargs):
        ...
        det_bboxes, det_labels = self.simple_test_bboxes(
            z, x, img_meta_x, gt_bboxes_z, proposal_list, self.test_cfg.rcnn, **kwargs
        )
    ...
    def aug_test(self, *args, **kwargs):
        raise NotImplementedError("aug_test is not implemented for QG_RCNN")
    ...
    def _process_gallary(self, img_x, img_meta_x, **kwargs):
        ...
        if not kwargs.get("keep_order", False):
            ...
            bbox_results = [
                np.concatenate(
                    [det_bboxes.cpu().numpy(), det_labels.cpu().numpy()[:, None]],
                    axis=1,
                )
            ]
